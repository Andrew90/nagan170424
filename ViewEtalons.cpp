...
void __fastcall TFormViewEtalons::ViewAllClick(TObject *Sender)
{
	...
	DM->QEtalons->Active=false;
	DM->QEtalons->SQL->Clear();
	DM->QEtalons->SQL->Add("Select * from dbo.SGEtalon");
	DM->QEtalons->ExecSQL();
	DM->QEtalons->Active=true;
...
void __fastcall TFormViewEtalons::txt1Click(TObject *Sender)
{
	if( OpenDialog->Execute() )
	{
		...
		else
		{
			...
			while(!feof(file))
			{
				...
				DM->tTemp->FieldByName("TypeSize")->AsString = Globals::current_typesize;
