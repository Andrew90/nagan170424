...
{
public:
	enum Types { sgUnknown=0, sgD, sgK, sgE };
	...
	const map< Group::Types, char > init_sgToEnglish();
	const map< Group::Types, int> init_sgToNumber();

	operator String();
	operator const String() const;

	const char GroupToEnglish() const;
...
private:
	...
	const static map< Group::Types, char > sgToEnglish;
...
class Tube : public ClusterPoint
{
 public:
	...
	Tube( vector<int>    sourceSignals, TubeType tubeType );
   ...
   //	Tube& operator = (const Tube& _tube);
	...
	void setSolidGroup( Group solidGroup )  { solidgroup = solidGroup; }

	vector<double> coordsFromSignal( vector<int> sourceSignals );
	// возвращает массив магнитных сигналов по конкретному порогу
	vector<int> getParameters(int par);
	// возвращает вектор пороговых значений
	vector<int> getThresholds();
	//устанавливает пороги по гп
	void setThresholds(vector<int> _thresholds) { thresholds = _thresholds; }
