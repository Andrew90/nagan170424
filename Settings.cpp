...
// изменился типоразмер - нужно показать его настройки. удаляем все поля и заполняем заново
	...
	try
	{
		...
		ValueListEditor->InsertRow("Скорость вращения продольного на входе",ini->ReadInteger(sect,"InSpeed",0),true);
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->ReadOnly = true;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->EditStyle=esPickList;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->PickList->AddStrings(Globals::speeds.getSpeedNames());
		ValueListEditor->InsertRow("Скорость вращения продольного в работе",ini->ReadInteger(sect,"LinearSpeed",0),true);
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->ReadOnly = true;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->EditStyle=esPickList;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->PickList->AddStrings(Globals::speeds.getSpeedNames());
		ValueListEditor->InsertRow("Скорость вращения продольного на выходе",ini->ReadInteger(sect,"OutSpeed",0),true);
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->ReadOnly = true;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->EditStyle=esPickList;
		ValueListEditor->ItemProps[ ValueListEditor->RowCount-2 ]->PickList->AddStrings(Globals::speeds.getSpeedNames());
