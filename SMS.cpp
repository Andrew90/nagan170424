...
String SMS::CreateServerRespond()
{
	...
	if (rec.SubString(1,5) == "zone ")
	{
		...
		MainForm->sum.ComputeZonesData(MainForm->pp , MainForm->pr , MainForm->th, true, true);
		MainForm->sum.PutResultOnChart(MainForm->SummaryChart , MainForm);
		return "zone ok";
	}
...
//	Server->ReceiveBuf(server_rec , sen_length , 0);
	...
	for (int i = 0; i < server_rec_length; i++)
		str += server_rec[i];
	ProtocolForm->SendToProtocol( str );
//	ProtocolForm->SendToProtocol("Сервер получил байт: " + IntToStr(len) + ", клиентсокет байт: " + IntToStr(server_rec_length) );
...
//	ProtocolForm->SendToProtocol("Сервер отправил данные" );
}
void __fastcall SMS::ServerCreateHandle(TObject *Sender)
{
	// журнал
	ProtocolForm->SendToProtocol("Сервер создан");
}
...
void __fastcall SMS::ServerSend(TObject *Sender, PChar Buf, int &DataLen)
{
	...
	for (int i = 0; i < DataLen; i++)
		str += (wchar_t) Buf[i];
	ProtocolForm->SendToProtocol(L"Сервер отправил данные: " + str + L", байт: " + IntToStr(len) );
}
void __fastcall SMS::ServerReceive(TObject *Sender, PChar Buf, int &DataLen)
{
	...
	for (int i = 0; i < DataLen; i++)
		str += (wchar_t) Buf[i];
	ProtocolForm->SendToProtocol(L"Сервер получил данные: " + str + L", байт: " + IntToStr(len) );
}
//---------------------------------------------------------------------------
//-------Функции клиента-----------------------------------------------------
...
{
	ProtocolForm->SendToProtocol("Клиент выдал ошибку " + IntToStr(SocketError));
