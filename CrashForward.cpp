...
// если модули пусты - то Транзит
			...
			!digital->CheckInSignal("стойка 4") )
	{
		...
		ProtocolForm->SendToProtocol("Режим Транзит");
...
// если где-то труба застряла - то Выгон
	else
	{
		...
		stext2 = "Начинаем работу";
		ProtocolForm->SendToProtocol("-----------------");
		ProtocolForm->SendToProtocol("Режим Выгон");
		Synchronize( &UpdateMainForm );
...
// труба застряла в поперечном, продольный уберем
		if ( cross && !lin )
		{
			...
			{
				stext2 = ModuleResult;
				ProtocolForm->SendToProtocol("Выгон: " + stext2);
				result = false;
				break;
			}
		}
...
// устанавливаем выходы на частотник транспорта, труба едет вперед
...
bool CrashForwardThread::Transit()
{
	...
	if ( ModuleResult != "ok")
	{
		...
		stext2 = ModuleResult;
		ProtocolForm->SendToProtocol("Транзит: " + stext2);
		Synchronize( &UpdateMainForm );
		return false;
	}
...
// устанавливаем разрешающие сигналы в цех
		ProtocolForm->SendToProtocol("Транзит: установили разрешающие сигналы" );
...
//	digital->ResetOutSignal("тс: STF");
	...
	if ( result)
	{
		...
		ProtocolForm->SendToProtocol("Транзит: " + stext1);
		Synchronize( &UpdateMainForm );
		return true;
	}
	else
