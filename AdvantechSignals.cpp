...
Advantech1730::Advantech1730()
{
  ...
  //	device=NULL;
   //	dio = NULL;
	...
	bouncePeriod = 2;		// время дребезга - интервал, через который будет проверяться состояние сигнала
...
bool Advantech1730::Init(long DevNum, bool ReadOnly)
{
 ErrorCode errorCode;
	...
	DeviceInformation devInfo(DevNum);
...
bool Advantech1730::SetOutWord(unsigned int OutWord)
{
	   ...
	   //Write DO ports’Status
	   errorCode = instantDoCtrl->Write(0,4,bufferForWriting);
	   //dio->DoWrite(startPort, portCount, bufferForWriting);
	   if (errorCode == Success)
	   {
		   TThread::CurrentThread->Sleep(SleepTime);
		   //Read back DO ports’Status
...
bool Advantech1730::CheckOutWord()
{
	...
	AdvMutex->Acquire();
	errorCode = instantDoCtrl->Read(0,4,bufferForReading);
	//dio->DoRead(startPort, portCount, bufferForReading);	//Read DO ports’status
	if (errorCode==Success)
	{
		DoValue=bufferForReading[0]+
			   (bufferForReading[1] << 8)+
...
bool Advantech1730::CheckInWord()
{
	...
	//dio->DiRead(startPort, portCount, bufferForReading);//Read DI ports’status
	if (errorCode==Success)
	{
...
bool Advantech1730::CheckInBouncelessBit(byte bit, bool check_status)
{
	...
	{
		result = ( CheckInBit(bit) == check_status );
...
//------------------------------------------------------------------------------
bool Advantech1730::WaitForInBit(byte bit, unsigned int DelayTimeMSec)
{
	for( DWORD tt = GetTickCount(); GetTickCount() - tt < DelayTimeMSec ;  )
	{
		if ( CheckInBit( bit ) )
			return true;
...
//------------------------------------------------------------------------------
bool Advantech1730::WaitForDropInBit(byte bit, unsigned int DelayTimeMSec)
{
	for( DWORD tt = GetTickCount(); GetTickCount() - tt < DelayTimeMSec ;  )
	{
		if ( ! CheckInBit( bit ) )
...
{
/*
	for( int tt = GetTickCount(); GetTickCount() - tt < DelayTimeMSec ;  )
	{
		if ( ! CheckInBit( bit ) )
			return true;
		if ( thread->CheckTerminated() )
...
AllSignals::AllSignals()
{
	...
	AdvCards[0]->Init(0, false);
	AdvCards[1]->Init(1, false);
}
//------------------------------------------------------------------------------
// очищаем память
AllSignals::~AllSignals()
...
//нахера искать когда они уже есть
...
bool AllSignals::CheckInSignal(String SignalName)
{
	...
	return AdvCards[ s.card ] -> CheckInBit( s.channel );
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// ждет входного сигнала определенное время, заданное в DelayTimeMsec в миллисекундах
bool AllSignals::WaitForInSignal(String SignalName, int DelayTimeMSec)
{
	Signal s = FindSignal(SignalName, inmap);
	return AdvCards[ s.card ] -> WaitForInBit( s.channel, DelayTimeMSec );
}
//------------------------------------------------------------------------------
// ждет пропадания входного сигнала определенное время, заданное в DelayTimeMsec в миллисекундах, возвращает TRUE если сигнал пропал
bool AllSignals::WaitForDropInSignal(String SignalName, int DelayTimeMSec)
{
	Signal s = FindSignal(SignalName, inmap);
	return AdvCards[ s.card ] -> WaitForDropInBit( s.channel, DelayTimeMSec );
}
//------------------------------------------------------------------------------
